...
function persona_creditos_select($cedula){
	$sql = "SELECT *   FROM administra a, credito c WHERE personas_cedula = $cedula AND  egresos = 'CREDITOS' AND c.administra_idadministra = a.idadministra AND c.estatus = 'ABIERTO'
	 ORDER BY c.idcredito DESC";
	$ej = mysql_query($sql);
	$var = '';
	while($dt = mysql_fetch_array($ej)){	
		$var .= '<option value="'.$dt['idcredito'].'">'.$dt['concepto'].'</option>';
	}
	if($var == ''){
		return '<option value="-1">No hay créditos pendientes.</option>';
	}
	return $var;
}
...
function persona_ajustes($cedula, $lima){
	...
	$sql = "SELECT * FROM administra
		WHERE personas_cedula = $cedula AND  (ingresos = 'REVERSOS' OR egresos = 'REVERSOS' OR ingresos = 'AJUSTES' OR egresos = 'AJUSTES')
